...
public interface BoardMapper {
	...
	// 리턴 자료형은 글 하나를 담장할 수 있는 BoardVO로 해야함
	...
	// 글 수정은 UPDATE구문으로 합니다. 메서드를 선언해주세요.
	// 전달변수가 title, content, bno이므로, 단일자료가 아닌 묶음으로 전달합니다.
	public void update(BoardVO vo);
	
	
	// vo안쓰고 데이터 전달하기
	// 2개이상의 파라미터를 따로따로 전달할때는 각 파라미터 왼쪽에 @Param을 븉여줍니다
	public void update2(@Param("title")String title, 
						@Param("content")String content, 
						@Param("bno") long bno);
	
	// 전체 글 개수를 얻어오는 getPageNum을 선언합니다.
	// 파라미터는 필요없습니다.
	// 글개수 => 정수값을 조회하기 때문에 int 리턴입니다.
