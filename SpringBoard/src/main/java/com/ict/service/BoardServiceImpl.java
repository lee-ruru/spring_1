...
public class BoardServiceImpl implements BoardService{
	...
	// 그러면, 그 메서드를 보유하고 있는클래스를 선언하고 주입해줘야 합니다.
	// DB접근시 사용하는 BoardMapper를 선언하고 주입해줘야 합니다.
	// 참고)BoardMapperTest.java
	@Autowired
	private BoardMapper boardMapper;//(실제로 주입되는 요소는 ServiceImpl이므로 호출지는 
	
	//글삭제시 해당글의 전체 댓글을 삭제하기 위해 reply_tbl 생성
	@Autowired
	...
	public void insert(BoardVO vo) {
		log.info("이번에 새로쓰는 글번호 : " + vo.getBno());
	...
	public void delete(long bno) {
		...
		// mapper를 이용해 구현
		//글마저 삭제(댓글이 없는 글만 삭제가 가능하므로 위의 순서대로 작성)
