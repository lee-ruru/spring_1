...
public class PageMaker {
	...
	public void calcData() {
		...
		// 위의 endPage의 명목상의(단순 그룹계산으로) 끝나는 페이지이기 때문에 실질적인 글 개수를 통해 보정해줘야함
		...
		// next, prev 버튼 여부처리
		// prev는 startPage가 1인 경우에만 비활성화이므로 삼항연산자로 간단히 처리합니다.
		prev = startPage == 1 ? false : true;
		
		// next는 여태까지 출력한 페이지에 속한 글 개수 보다 DB내 전체 글이 더 많은 경우에 활성화
	...
	public void setTotalBoard(int totalBoard) {
		...
		calcData();// prev, next, endPage, startPage를 구해버림.
	}
